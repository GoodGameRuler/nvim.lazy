  ...
  config = function()
    require('gitsigns').setup({
      on_attach = function(bufnr)
        ...
        map('n', '<leader>hl', gitsigns.toggle_current_line_blame, { desc = "Git Blame Line" })
        map('n', '<leader>hd', gitsigns.diffthis, { desc = "Git Diff" })
        map('n', '<leader>hD', function() gitsigns.diffthis('~') end, { desc = "Git Diff Unknown" })
        map('n', '<leader>hx', gitsigns.toggle_deleted, { desc = "Git Toggle Deleted" })

        -- Text object
        map({ 'o', 'x' }, 'hy', ':<C-U>Gitsigns select_hunk<CR>', { desc = "Git Signs Select Hunk" })
      end
    }
    )
  end
